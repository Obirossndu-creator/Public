...
const uiText = {
  en: {
    ...
    navContact: "Contact",
    heroBadge: "BIM Engineer • Structural Engineering • Digital Construction • Risk Analysis",
    ...
    <div className="mt-6 grid gap-3 md:grid-cols-2">
      {cards.map((card) => (
        <div key={card.title} className="group overflow-hidden rounded-2xl border border-white/10 bg-black/20">
          <div className={`relative h-52 overflow-hidden bg-gradient-to-br ${card.gradient}`}>
            {card.image ? (
              <img
                src={card.image}
                alt={card.title}
